...
## Analysis
# 1. Merging the training and test dataset into one.
dataSet <- rbind(X_train,X_test)

# 2. Extracting the mean and standard deviations
# Create a vector of only mean and std, use the vector to subset.
meanandstdonly <- grep("mean()|std()", features[, 2]) 
dataSet <- dataSet[,meanandstdonly]


# 4. Appropriately labeleling the data set with descriptive activity names.
# Create vector of "Clean" feature names by getting rid of "()" apply to the dataSet to rename labels.
CleanFeatureNames <- sapply(features[, 2], function(x) {gsub("[()]", "",x)})
names(dataSet) <- CleanFeatureNames[meanandstdonly]

# Combining test and train of subject data and activity data, and adding descriptive lables.
subject <- rbind(subject_train, subject_test)
names(subject) <- 'subject'
activity <- rbind(y_train, y_test)
names(activity) <- 'activity'

# combining subject, activity, and mean and std only data set to create final data set.
